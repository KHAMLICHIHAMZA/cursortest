...
**Nom canonique :** `CURRENT_STATUS.md` (remplace l’ancien `STATUT_PREPROD.md`).
...
## Préprod actuelle — où faire les tests de recette

| Rôle | URL |
|------|-----|
| **Front web (référence recette / UAT)** | **https://v0-cursortest.vercel.app** |

Les **tests manuels, UAT et parcours métier** (checklists produit, démos) doivent être réalisés **sur ce déploiement Vercel**, pas seulement en local (`localhost`). La CI sur GitHub valide le **code** ; la **preuve côté utilisateur** se fait sur cette URL.

**Côté Vercel :** `NEXT_PUBLIC_API_URL` doit pointer vers l’**API publique** utilisée pour cette préprod (suffixe `/api/v1` selon votre config). Le **smoke API** (`SMOKE_API_BASE` dans `backend/scripts/smoke-remote-api.mjs`) cible l’URL **backend**, pas le front — voir [`docs/PRODUCTION.md`](./docs/PRODUCTION.md).
