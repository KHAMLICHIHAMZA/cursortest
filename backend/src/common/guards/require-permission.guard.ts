...
export class RequirePermissionGuard implements CanActivate {
  ...
  async canActivate(context: ExecutionContext): Promise<boolean> {
    const requiredPermission =
      this.reflector.getAllAndOverride<UserAgencyPermission>(
    ...
    if (!agencyId) {
      throw new BadRequestException(
        "L'identifiant de l'agence est requis pour la vérification des permissions",
      );
    }
    ...
    if (!userAgency) {
      throw new ForbiddenException(
        "Vous n'avez pas accès à cette agence. Vérifiez votre rattachement.",
      );
    }
  ...
  private checkPermission(
    ...
    return (
      permissionLevel[userPermission] >= permissionLevel[requiredPermission]
    );
  }
}
