...
describe("BookingService", () => {
  ...
  const mockCommonAuditService = {
    addCreateAuditFields: jest.fn((data, userId) => ({
      ...data,
      createdByUserId: userId,
    })),
    addUpdateAuditFields: jest.fn((data, userId) => ({
      ...data,
  ...
  beforeEach(async () => {
    const module: TestingModule = await Test.createTestingModule({
      providers: [
        ...
        {
          provide: BusinessEventLogService,
          useValue: mockBusinessEventLogService,
        },
  ...
  describe("create", () => {
    ...
    it("should create booking when all validations pass", async () => {
      ...
      mockPrismaService.bookingNumberSequence.upsert.mockResolvedValue({
        lastValue: 1,
      });
    ...
    it("should reject MANUAL bookingNumber with non-alphanumeric characters", async () => {
      ...
      const mockAgency = {
        ...
        companyId: "company-1",
        preparationTimeMinutes: 60,
        company: { id: "company-1", bookingNumberMode: "MANUAL" },
      };
  ...
  describe("isValidStatusTransition", () => {
    it("should return true for valid transitions", () => {
      ...
      expect(service["isValidStatusTransition"]("PENDING", "CONFIRMED")).toBe(
        true,
      );
      expect(
        service["isValidStatusTransition"]("CONFIRMED", "IN_PROGRESS"),
      ).toBe(true);
      expect(
    ...
    it("should return false for invalid transitions", () => {
      expect(service["isValidStatusTransition"]("DRAFT", "RETURNED")).toBe(
        false,
      );
      expect(service["isValidStatusTransition"]("RETURNED", "CONFIRMED")).toBe(
        false,
      );
      expect(
        service["isValidStatusTransition"]("CANCELLED", "IN_PROGRESS"),
      ).toBe(false);
    });
  });
  ...
  describe("V2 BookingNumber AUTO (concurrency)", () => {
    it("should generate unique booking numbers for concurrent calls", async () => {
      ...
      let counter = 0;
      mockPrismaService.bookingNumberSequence.upsert.mockImplementation(
        async () => {
          counter += 1;
      ...
      expect(
        mockPrismaService.bookingNumberSequence.upsert,
