...
import {
  licenseImageStorage,
  licenseImageFilter,
} from "./interceptors/license-upload.interceptor";
...
export class ClientController {
  ...
  private extractAddressFromNote(note?: string | null): string | undefined {
    if (!note || !note.includes("Adresse:")) return undefined;
    const match = note.match(
      /Adresse:\s*(.*?)(?:,\s*(?:Permis:|Type permis:)|$)/,
    );
  ...
  async findAll(
    ...
    return clients.map((client) => {
      ...
      return {
        ...
        address: this.extractAddressFromNote(client.note),
        licenseNumber:
          client.licenseNumber ||
          (client.note?.includes("Permis:")
