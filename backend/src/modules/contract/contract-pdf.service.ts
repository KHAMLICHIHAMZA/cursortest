    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(9)
          .font("Helvetica")
          .text(`Version ${contractVersion} | Statut: ${status}`, {
            align: "center",
          });
        doc.moveDown(1.5);
        ...
        doc.text(
          `LE LOUEUR: ${payload.company.raisonSociale || payload.company.name}`,
        );
        ...
        if (payload.agency.address)
          doc.text(`Adresse: ${payload.agency.address}`);
        ...
        if (payload.client.idCardNumber)
          doc.text(`CIN: ${payload.client.idCardNumber}`);
        if (payload.client.passportNumber)
          doc.text(`Passeport: ${payload.client.passportNumber}`);
        ...
        doc
          .fontSize(12)
          .font("Helvetica-Bold")
          .text("Article 3 - Duree et Tarif");
        doc.moveDown(0.5);
        doc.fontSize(9).font("Helvetica");
        if (payload.booking.bookingNumber)
          doc.text(`N° Reservation: ${payload.booking.bookingNumber}`);
        doc.text(
          `Date de debut: ${this.formatDate(payload.booking.startDate)}`,
        );
        ...
        doc.text(
          "  - Utiliser le vehicule conformement a sa destination normale",
        );
        ...
        doc
          .fontSize(12)
          .font("Helvetica-Bold")
          .text("Article 5 - Conditions Generales");
        doc.moveDown(0.5);
        doc.fontSize(9).font("Helvetica");
        doc.text(
          "Tout retard de restitution entraine des frais supplementaires.",
        );
        doc.text(
          "En cas de sinistre, le locataire doit informer le loueur dans les 24 heures.",
        );
        ...
        if (payload.signatures?.clientSignedAt) {
          doc.text(
            `Signe le: ${this.formatDate(payload.signatures.clientSignedAt)}`,
            50,
          );
        ...
        if (payload.signatures?.agentSignedAt) {
          doc.text(
            `Signe le: ${this.formatDate(payload.signatures.agentSignedAt)}`,
            320,
          );
        ...
        doc.text(
          "Document genere par MalocAuto - Ce contrat a valeur de preuve.",
          50,
          undefined,
          { align: "center" },
        );
