...
export class GpsKpiService {
  ...
  async computeKpi(
    ...
    const gpsMissingRate =
      totalSnapshots > 0 ? (gpsMissingCount / totalSnapshots) * 100 : 0;
    const avgAccuracy =
      accuracyCount > 0
    ...
    for (const [vehicleId, vSnapshots] of vehicleMap) {
      ...
      const checkIns = vSnapshots.filter(
        (s: any) => s.reason === "CHECK_IN" && s.mileage != null,
      );
      const checkOuts = vSnapshots.filter(
        (s: any) => s.reason === "CHECK_OUT" && s.mileage != null,
      );
      ...
      if (checkOuts.length > 0)
        checkOutMileage = checkOuts[checkOuts.length - 1].mileage;
        ...
        if (mileageDelta > 0) {
          ...
          if (bookingSnapPairs.length >= 2) {
            ...
            const lastDate = new Date(
              bookingSnapPairs[bookingSnapPairs.length - 1].createdAt,
            );
            const days = Math.max(
              1,
    ...
    return {
      ...
      avgAccuracy,
      distanceEstimates: distanceEstimates.sort(
        (a, b) => (b.mileageDelta || 0) - (a.mileageDelta || 0),
      ),
      consistencyIssues,
    };
  }
