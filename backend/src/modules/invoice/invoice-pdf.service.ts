    ...
    return new Promise((resolve, reject) => {
      try {
        ...
        doc
          .fontSize(10)
          .font("Helvetica")
          .text(`N\u00b0 ${invoiceNumber}`, { align: "center" });
        doc.moveDown(0.3);
        doc.fontSize(9).text(`Date: ${this.formatDate(payload.issuedAt)}`, {
          align: "center",
        });
        doc.moveDown(1.5);
        ...
        if (payload.client.idCardNumber)
          doc.text(`CIN: ${payload.client.idCardNumber}`, 320);
        if (payload.client.passportNumber)
          doc.text(`Passeport: ${payload.client.passportNumber}`, 320);
        ...
        doc.text(
          `Vehicule: ${payload.vehicle.brand} ${payload.vehicle.model} - ${payload.vehicle.registrationNumber}`,
        );
        doc.text(
          `Du: ${this.formatDate(payload.booking.startDate)}  Au: ${this.formatDate(payload.booking.endDate)}`,
        );
        ...
        doc.text(`P.U. (${currency})`, 370, tableTop, {
          width: 80,
          align: "right",
        });
        doc.text(`Total (${currency})`, 460, tableTop, {
          width: 85,
          align: "right",
        });
        doc.moveDown(0.5);
        ...
        doc.text(`Location vehicule (${durationDays}j)`, 50, rowY, {
          width: 250,
        });
        doc.text(`${durationDays}`, 310, rowY, { width: 50, align: "center" });
        doc.text(`${dailyRate.toFixed(2)}`, 370, rowY, {
          width: 80,
          align: "right",
        });
        ...
        if (payload.amounts.lateFees > 0) {
          ...
          doc.text(`${payload.amounts.lateFees.toFixed(2)}`, 370, feeRowY, {
            width: 80,
            align: "right",
          });
          doc.text(`${payload.amounts.lateFees.toFixed(2)}`, 460, feeRowY, {
            width: 85,
            align: "right",
          });
          doc.moveDown(1);
        ...
        if (payload.amounts.lateFees > 0) {
          ...
          doc.text(
            `${payload.amounts.lateFees.toFixed(2)} ${currency}`,
            460,
            doc.y - doc.currentLineHeight(),
            { width: 85, align: "right" },
          );
        ...
        doc.text(
          `${payload.amounts.total.toFixed(2)} ${currency}`,
          460,
          doc.y - doc.currentLineHeight(),
          { width: 85, align: "right" },
        );
        ...
        // Deposit info
        if (
          payload.booking.depositAmount &&
          payload.booking.depositAmount > 0
        ) {
          doc.font("Helvetica").fontSize(9);
          doc.text(
            `Caution: ${payload.booking.depositAmount.toFixed(2)} ${currency} (${payload.booking.depositStatusFinal || "N/A"})`,
            50,
          );
          doc.moveDown(0.5);
        }
        ...
        doc.text(
          "Document genere par MalocAuto - Logiciel de gestion de location de vehicules",
          { align: "center" },
        );
        doc.text(
          `Devise: ${payload.amounts.currency} | Timezone: ${payload.timezone}`,
          { align: "center" },
        );
        if (isCredit) {
          doc.moveDown(0.3);
          doc.text("Ce document constitue un avoir sur la facture originale.", {
            align: "center",
          });
        }
