...
export class PushService {
  ...
  constructor(
    ...
    if (!this.fcmEnabled) {
      this.logger.warn(
        "FCM non configuré - Les notifications push seront enregistrées mais non envoyées",
      );
    }
  }
    ...
    if (serviceAccountJson || serviceAccountPath) {
      try {
        ...
        try {
          ...
          this.logger.debug(
            "firebase-admin SDK not installed, using legacy server key method",
          );
        ...
        this.logger.warn(
          "OAuth2 initialization failed, falling back to legacy server key method",
          error.message,
        );
      }
    }
