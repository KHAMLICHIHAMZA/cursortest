...
describe("PlanService", () => {
  ...
  describe("findOne", () => {
    ...
    it("should return plan with modules and quotas", async () => {
      const mockPlan = {
        ...
        planQuotas: [{ planId, quotaKey: "agencies", quotaValue: 10 }],
        _count: { subscriptions: 5 },
      };
  ...
  describe("update", () => {
    ...
    it("should update plan successfully", async () => {
      ...
      expect(result).toBeDefined();
      expect(mockAuditService.addUpdateAuditFields).toHaveBeenCalledWith(
        updateDto,
        "user-1",
      );
  ...
  describe("remove", () => {
    ...
    it("should throw NotFoundException if plan not found", async () => {
      ...
      await expect(service.remove(planId, mockUser)).rejects.toThrow(
        NotFoundException,
      );
    });
