...
describe("SubscriptionService", () => {
  ...
  describe("create", () => {
    ...
    it("should create subscription successfully", async () => {
      const mockPlan = {
        ...
        price: 1000,
        isActive: true,
        planModules: [{ moduleCode: "VEHICLES" }, { moduleCode: "BOOKINGS" }],
      };
      ...
      mockPrismaService.subscriptionModule.createMany.mockResolvedValue({
        count: 2,
      });
      mockPrismaService.companyModule.createMany.mockResolvedValue({
        count: 2,
      });
      ...
      expect(
        mockPrismaService.subscriptionModule.createMany,
      ).toHaveBeenCalled();
  ...
  describe("suspend", () => {
    it("should throw ForbiddenException if user is not SUPER_ADMIN", async () => {
      ...
      await expect(
        service.suspend("sub-1", "Reason", mockUser),
  ...
  describe("restore", () => {
    it("should throw BadRequestException if subscription is not suspended", async () => {
      ...
      await expect(service.restore("sub-1", mockUser)).rejects.toThrow(
        BadRequestException,
      );
    });
